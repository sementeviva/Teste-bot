...
def login():
    """Gere o processo de login para os seus clientes (donos de loja)."""
...
# --- ROTA DE REGISTO SEGURA E SIMPLIFICADA ---
@auth_bp.route('/registro', methods=['GET', 'POST'])
def registro():
    """
    Gere o registo de uma nova conta. A rota é protegida por uma chave secreta
    passada como um parâmetro na URL (?token=...).
    """
    secret_key = os.environ.get('REGISTRATION_SECRET_KEY')
    token_from_url = request.args.get('token')

    # A verificação de segurança: a chave existe e corresponde à do URL?
    if not secret_key or token_from_url != secret_key:
        flash('Acesso negado. Esta é uma área protegida.', 'danger')
        return redirect(url_for('auth.login'))

    # Se a chave estiver correta, a lógica do formulário continua.
    if request.method == 'POST':
        ...
        if password != password_confirm:
            ...
            return redirect(url_for('auth.registro', token=secret_key))
    ...
    # Se for um acesso GET (com a chave correta), mostra a página de registo.
